...
export class ChatComponent  {
    ...
    const currentTime = new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
        ...
        //user|_id, reciever_id,content
         if (this.content.trim() ){
        this.userService.addChat(user_id,this.activeUser.profile_id,this.content.trim() || '').subscribe({
            next:(response)=>{
           // Only create a message if there is text or a file
        if (this.content.trim() || this.selectedFile) {

                console.log(response.data.content);
                  // Assuming the response is the message object, push it to the messages array 
